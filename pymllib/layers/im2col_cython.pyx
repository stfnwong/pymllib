"""
IM2COL_CYTHON
Cython implementation of im2col. This is taken from CS231n
"""
...
cdef int im2col_cython_inner(np.ndarray[DTYPE_t, ndim=2] cols,
                             np.ndarray[DTYPE_t, ndim=3] x_padded,
...
def im2col_cython(np.ndarray[DTYPE_t, ndim=4] x, int field_height,
    ...
    cdef int HH = 1 + (H + 2 * padding - field_height) / stride
    cdef int WW = 1 + (W + 2 * padding - field_width) / stride
    cdef int p = padding
...
def col2im_cython(np.ndarray[DTYPE_t, ndim=2] cols,
